 ...
 * Description  : ������ie������ȥ��P2P IE���������������ie_out��
 * Input        : ie
 *              : ie_len
 * Output       : ie_out,��Ҫ�����߱�֤ ��Ϊ�գ������㹻�ռ�洢���IE
 *              : ie_out_len ȥ��P2P IE���IE����
 * Return Value : int32
 * Calls        :
 * Called By    :
 *
...
int32 hwifi_fit_cpufreq(struct cfg_struct *cfg, uint8 value)
{
    ...
    /**
     *����fit_freq bin WID,wid����Ϊ1ʱ��Ϊ��Ƶwid
     */
...
int32 hwifi_shutdown_RF(struct cfg_struct *cfg)
{
    ...
    /**
     *����WID_FREQ_PLAN bin WID,wid����Ϊ1����ֵΪ0ʱ��Ϊshutdown RF wid
     */
...
int32 hwifi_shutdown_powersave(struct cfg_struct *cfg)
{
    ...
    /**
     *����WID_FREQ_PLAN bin WID,wid����Ϊ1����ֵΪ0xFEʱ��Ϊshutdown powersave wid
     */
...
int32 hwifi_start_addset_beacon(struct cfg_struct *cfg, struct hwifi_beacon_param_stru  *beacon_param)
{
    ...
    /* WID_11I_MODE,�ӿ��ĵ�ΪWID_11I_ENCYPT_MODE?? */
...
void hwifi_get_rate_info(DEVICE_STATS_STRU stStatInfo, struct rate_info *rate_info)
{
    ...
    if(stStatInfo.ucTxRateIdx >> 4 == 0x8)/*���ҵ�11n����������*/
    {
        int index;
        for(index = 0; index < sizeof(stTxRate)/sizeof(TX_RATE_STRU); index++) /*���ƥ���Ӧ������ֵ*/
        {
...
int32 hwifi_recv_status_ready(struct cfg_struct *cfg, uint8 *buf)
{
    ...
    cfg->hi110x_dev->mode.current_mode  = (uint8)(OS_LE16_TO_CPU(msg->ucMode)); /* Deviceģʽ */
...
int32 hwifi_proc_res_msg(struct cfg_struct *cfg, uint8 *buf)
{
    ...
    /* ��̵�wid��char���͵ģ�1�β�ѯ���ϱ����wid��ѯ��� */
    pos = (uint8 *)(msghdr->wid_list);
    while(msg_len >= sizeof(struct char_wid))
    {
        /* ����TLV ��ʽ�Ĳ�ѯ���,type:2�ֽڣ�len:1�ֽڣ�Value����lenֵ���� */
        wid = pos[0] | (pos[1] << 8);
        wid_value_len = pos[2];
        ...
        msg_len = msg_len - wid_value_len - 3; /* TLV ��ʽ��TYPE �� len Ϊ3�ֽ� */
                         ...
                         uint16             asoc_rsp_ie_len)
{
    ...
    {
        /* STA����P2P ɨ��״̬��ֻ�����probe_rsp_ie��Ϣ��
         * �������IEΪ�գ������P2P ����״̬���õ�WPS/P2P IE ��
         * ���ֻ��probe response��Ϊ�գ����·�����WPS/P2P IE,����device��Ӧprobe responseЯ����IE
         */
         msg_size = sizeof(struct hwifi_msg_header)
    ...
    {
        /* GO�������е�P2P/WPS IE
         * ��ȻGO�ڳ�ʼ����ȥ��ʼ��ʱҲ��������е�IE��Ϣ,����device����ģʽ�л������и����������
         */
        msg_size = sizeof(struct hwifi_msg_header)
